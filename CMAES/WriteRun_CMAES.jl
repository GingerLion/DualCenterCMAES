...
function write_run_header(fileName::String, returnInfo::ReReturnInfo, prefixNames::Vector{String}, sep::String)
  open(fileName, "w") do f
    ...
    write(f, "restart$(sep)")
    write(f, "lambda$(sep)")
    #write(f, "lambda_bug$(sep)")
    write(f, "gen$(sep)")
    #write(f, "gen_bug$(sep)")
    write(f, "evals$(sep)")
    write(f, "cond$(sep)")
    write(f, "cond_bug$(sep)")
    write(f, "sigma$(sep)")
    write(f, "sigma_bug$(sep)")
    #write(f, "eig_min$(sep)")
    #write(f, "eig_max$(sep)")
    ...
    write(f, "||p_sig||$(sep)")
    write(f, "c_sig/d_sig_bug$(sep)")
    write(f, "||p_sig||_bug$(sep)")
    ...
    write(f, "best_fit_bug$(sep)")
    write(f, "centerFit$(sep)")
    write(f, "centerFitBug$(sep)")
    write(f, "center$(sep)")
    write(f, "center_bug$(sep)")
    #write(f, "B$(sep)")
    #write(f, "D$(sep)")
    write(f, "BD$(sep)")
    write(f, "BD_bug$(sep)")
...
function write_run(fileName::String, returnInfo::ReReturnInfo, sys::CMAES_System,
  ...
  open(fileName, "a") do f
    for i = 1:(length(ri)-1)
      ...
      write(f, "$(ri[:σ_part1_bug][i])$sep")
      write(f, "$(ri[:σ_part2_bug][i])$sep")
