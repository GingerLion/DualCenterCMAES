...
function write_run_header(fileName::String, returnInfo::ReReturnInfo, prefixNames::Vector{String}, sep::String)
  open(fileName, "w") do f
    ...
    #vectors
    write(f, "orig_scale$(sep)")
    write(f, "best_scale$(sep)")
    write(f, "bestFit_overall$(sep)")
    write(f, "bestFitShadow_overall$(sep)")
    write(f, "centerFit$(sep)")
    write(f, "centerFitShadow$(sep)")
    write(f, "centerFitShadow_$(sep)")
    write(f, "dcs_centerDiff$(sep)")
    write(f, "mahalanobis_centerDiff$(sep)")
    write(f, "src_bestFitShadow_$(sep)")
    write(f, "center_if_fit\n")
    #write(f, "center_if\n")
    #write(f, "orig_upper$(sep)")
    #write(f, "best_upper$(sep)")
    #write(f, "orig_middle$(sep)")
    #write(f, "best_middle$(sep)")
    #write(f, "orig_lower$(sep)")
    #write(f, "best_lower\n")
    #write(f, "center$(sep)")
    #write(f, "center_shadow$(sep)")
    #write(f, "center_shadow_$(sep)")
    #write(f, "B$(sep)")
    #write(f, "D$(sep)")
    #write(f, "BD$(sep)")
  ...
  open(fileName, "a") do f
    for i = 1:(length(ri)-1)
     ...
     # write(f, "$(ri[:σ_part6][i])$sep")
      ...
      write(f, "$(ri[:orig_scale][i])$sep")
      write(f, "$(ri[:best_scale][i])$sep")
      write(f, "$(ri[:best_overall][i])$sep")
      write(f, "$(ri[:best_overall_][i])$sep")
      write(f, "$(ri[:center_fit][i])$sep")
      write(f, "$(ri[:center_fit_shadow][i])$sep")
      write(f, "$(ri[:center_fit_shadow_][i])$sep")
      write(f, "$(ri[:euc_centerDiff][i])$sep")
      write(f, "$(ri[:mahalanobis_centerDiff][i])$sep")
      write(f, "$(ri[:center_shadow_source][i])$sep")
      write(f, "$(ri[:center_if_fit][i])\n")
      #write(f, "$(ri[:center_if][i])\n")
      #write(f, "$(ri[:orig_upper][i])$sep")
      #write(f, "$(ri[:best_upper][i])$sep")
      #write(f, "$(ri[:orig_middle][i])$sep")
      #write(f, "$(ri[:best_middle][i])$sep")
      #write(f, "$(ri[:orig_lower][i])$sep")
      #write(f, "$(ri[:best_lower][i])\n")
      #write(f, "$(ri[:center][i][1])$sep")
      #write(f, "$(ri[:center_shadow][i][1])$sep")
      #write(f, "$(ri[:center_shadow_][i][1])$sep")
...
function write_sourcesummary(f::IO, source::SelectionSource, sep::String)
	#write_proportions(f, source, sep) #used for selection source of old style elitism (μ+λ)
    write_proportions_species(f, source, sep) #used for dual center
